...
WORKDIR /src
COPY ["ProcessCreditCard.API.csproj", "ProcessCreditCard.API/"]
RUN dotnet restore "ProcessCreditCard.API/ProcessCreditCard.API.csproj"

WORKDIR "/src/ProcessCreditCard.API"
COPY . .
RUN dotnet build "ProcessCreditCard.API.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "ProcessCreditCard.API.csproj" \
...
Execute unit tests
```
    cd "ProcessCreditCard.API/test/ProcessCreditCard.API.Tests"
    dotnet test
```

Deploy application
```
    cd "ProcessCreditCard.API/src/ProcessCreditCard.API"
